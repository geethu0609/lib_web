...
const { getFacultyChannels, activateRoom, deactivateRoom, getRoomEvents, dismissEvent, informAdmin } = require('../controllers/facultyController');

router.use(auth, requireRole('faculty'));
router.get('/channels', getFacultyChannels);
router.post('/rooms/:id/activate', activateRoom);
router.post('/rooms/:id/deactivate', deactivateRoom);
